...
@dataclass
class Config:
    experiment_name: str = "20231226_01"
    n_fold: int = 5
    seed: int = 0
...
# %%
oof_preds_lgb = (
    oof_preds_lgb_label1 - oof_preds_lgb_label2
)
...
# %%
np.quantile(oof_preds_lgb, 1 - 0.788181), np.quantile(oof_preds_lgb, 0.034928)

# %%
calc_f1(
    (np.quantile(oof_preds_lgb, 0.034928), np.quantile(oof_preds_lgb, 1 - 0.788181)),
    oof_preds_lgb.tolist(), train["health"].tolist(),
)
...
def calc_f1_with_linear_coef(param, label0_pred, label1_pred, label2_pred):
    oof_preds = (
        np.array(label0_pred) * param[0] + np.array(label1_pred) * param[1] + np.array(label2_pred) * param[2]
    )
    return calc_f1(
        (np.quantile(oof_preds_lgb, 0.034928), np.quantile(oof_preds_lgb, 1 - 0.788181)),
        oof_preds.tolist(), train["health"].tolist(),
    )

para = [1, 2, -1]

m = optimize.minimize(
    calc_f1_with_linear_coef, para,
    args=(oof_preds_lgb_label0.tolist(), oof_preds_lgb_label1.tolist(), oof_preds_lgb_label2.tolist(), ),
    method="Nelder-Mead"
)
