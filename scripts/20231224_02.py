...
@dataclass
class Config:
    experiment_name: str = "20231224_02"
    n_fold: int = 5
    seed: int = 0
...
# %%
train.loc[train["health"] == 2, "health"] = -1
train["health"] += 1
train["health"].describe()
...
def calc_f1(param, x, y):
    arr = np.zeros_like(x)
    _x = np.array(x)
    arr[np.where(_x < para[0])] = 2
    arr[np.where((para[0] <= _x) & (_x < para[1]))] = 0
    arr[np.where(para[1] <= _x)] = 1
...
# %% [markdown]
# - rmse no weight: 0.29384823469054616
# - mae no weight: 0.31838683076810625
# - rmse with weight: 0.11565345790558905
# - mae with weight (lr=0.1): 0.11761172011550852
